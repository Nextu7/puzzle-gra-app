...
class SliderComponent extends HTMLElement {
  constructor() {
    ...
    if (this.sliderControlLinksArray.length > 0) {
  ...
  initPages() {
    ...
    const isProductGallery = this.slider.closest('.product__media-wrapper');

    if (isProductGallery && window.matchMedia('(max-width: 749px)').matches) {
  ...
  update() {
    ...
    // -- POCZĄTEK DODANEGO KODU (AKTUALIZACJA AKTYWNYCH KROPEK/NUMERÓW) --
    if (this.sliderControlLinksArray.length > 0) {
        this.sliderControlLinksArray.forEach((link) => {
            link.classList.remove('slider-counter__link--active');
            link.removeAttribute('aria-current');
        });
        if(this.sliderControlLinksArray[this.currentPage - 1]) {
            this.sliderControlLinksArray[this.currentPage - 1].classList.add('slider-counter__link--active');
            this.sliderControlLinksArray[this.currentPage - 1].setAttribute('aria-current', true);
        }
    }
    // -- KONIEC DODANEGO KODU --
  ...
  // -- POCZĄTEK DODANEJ FUNKCJI --
  linkToSlide(event) {
    event.preventDefault();
    const firstItemNode = this.slider.querySelector('[id^="Slide-"]');
